...
public class SingleCommandSender implements ConsoleCommandSender {
    ...
    public void sendMessage(String message) {
        ...
        Legacy.sendMessage(channel, message, false, Legacy.plugin.getConfig().getInt("DiscordChatChannelConsoleCommandExpiration") * 1000);

        // expire request after specified time
        if (Legacy.plugin.getConfig().getInt("DiscordChatChannelConsoleCommandExpiration") > 0 && Legacy.plugin.getConfig().getBoolean("DiscordChatChannelConsoleCommandExpirationDeleteRequest")) {
            try { Thread.sleep(Legacy.plugin.getConfig().getInt("DiscordChatChannelConsoleCommandExpiration") * 1000); } catch (InterruptedException e) { e.printStackTrace(); }
            if (event.getChannel().checkPermission(Legacy.jda.getSelfInfo(), net.dv8tion.jda.Permission.MESSAGE_MANAGE)) event.getMessage().deleteMessage(); else Legacy.plugin.getLogger().warning("Could not delete message in channel " + event.getChannel() + ", no permission to manage messages");
        }
    }
