...
import com.scarsz.discordsrv.todo.hooks.worlds.MultiverseCoreHook;
import com.scarsz.discordsrv.todo.listeners.*;
import com.scarsz.discordsrv.todo.objects.*;
...
import com.scarsz.discordsrv.todo.util.VanishedPlayerCheck;
...
public class Legacy extends JavaPlugin {
    ...
    public static void sendMessage(TextChannel channel, String message, boolean editMessage, int expiration) {
        ...
        if (editMessage)
            for (String phrase : Legacy.plugin.getConfig().getStringList("DiscordChatChannelCutPhrases"))
        ...
        channel.sendMessageAsync(message, m -> {
            if (expiration > 0) {
                ...
                if (channel.checkPermission(Legacy.jda.getSelfInfo(), Permission.MESSAGE_MANAGE)) m.deleteMessage(); else Legacy.plugin.getLogger().warning("Could not delete message in channel " + channel + ", no permission to manage messages");
            }
        });
    ...
    private static String getPrimaryGroup(Player player) {
        ...
        RegisteredServiceProvider<net.milkbowl.vault.permission.Permission> service = Legacy.plugin.getServer().getServicesManager().getRegistration(net.milkbowl.vault.permission.Permission.class);
    ...
    public static String getAllRoles(GuildMessageReceivedEvent event) {
        ...
        for (Role role : event.getGuild().getRolesForUser(event.getAuthor())) {
            roles += role.getName() + Legacy.plugin.getConfig().getString("DiscordToMinecraftAllRolesSeperator");
        }
        if (!roles.isEmpty()) roles = roles.substring(0, roles.length() - Legacy.plugin.getConfig().getString("DiscordToMinecraftAllRolesSeperator").length());
        return roles;
    }
    ...
    public static String convertRoleToMinecraftColor(Role role) {
        if (role == null) {
            if (Legacy.plugin.getConfig().getBoolean("ColorLookupDebug")) Legacy.plugin.getLogger().info("Role null, using no color");
            return "";
        }
        String colorHex = Integer.toHexString(role.getColor());
        String output = colors.get(colorHex);

        if (Legacy.plugin.getConfig().getBoolean("ColorLookupDebug")) Legacy.plugin.getLogger().info("Role " + role + " results to hex \"" + colorHex + "\" and output \"" + output + "\"");
